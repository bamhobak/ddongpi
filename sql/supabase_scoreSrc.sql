-- Per-object score share on each run, and its average in the dev stats.
-- Paste the whole file into Supabase -> SQL Editor and Run. Safe to run twice.
-- (No Korean literals here on purpose - the clipboard mangles them. Labels live in the game.)

alter table public.ddongpi_runs add column if not exists src jsonb;
