-- Record how many coins each run picked up, and whether the run evolved into
-- "full ownership", then report two separate averages in the summary.
...
alter table public.ddongpi_runs add column if not exists coins integer;
alter table public.ddongpi_runs add column if not exists fullown boolean;
  ...
  select jsonb_build_object(
    'summary', (
      select jsonb_build_object(
        ...
        -- coins: only runs that actually carry the number (older runs are null)
        'coin_runs',      count(*) filter (where coins is not null and fullown is not true),
        'avg_coins',      coalesce(round(avg(coins) filter
                            (where coins is not null and fullown is not true), 2), 0),
        'coin_runs_full', count(*) filter (where coins is not null and fullown is true),
        'avg_coins_full', coalesce(round(avg(coins) filter
                            (where coins is not null and fullown is true), 2), 0)
      ) from ddongpi_runs
    ),
